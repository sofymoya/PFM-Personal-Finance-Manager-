...
const Transactions: React.FC<{ userId: number }> = ({ userId }) => {
  ...
  const getFilteredTransactions = () => {
    ...
    return transactionsToFilter
      ...
      .sort((a, b) => new Date(b.fecha).getTime() - new Date(a.fecha).getTime()); // Ordenar del más reciente al más antiguo
  };
  ...
  const handleSaveTransactions = async (extractedTransactions: any[]) => {
    try {
  ...
  const prepararDatosGraficaBarras = () => {
    ...
    const meses = Object.entries(stats.statsPorMes).map(([key, data]: [string, any]) => ({
      key,
      ...data
    })).sort((a, b) => {
    ...
    return {
      labels: meses.map((m: any) => m.nombre),
      datasets: [
        {
          label: 'Gastos',
          data: meses.map((m: any) => m.gastos),
        ...
        {
          label: 'Ingresos',
          data: meses.map((m: any) => m.ingresos),
  ...
  const prepararDatosGraficaPastel = () => {
    ...
    const categorias = Object.entries(stats.statsPorCategoria)
      .filter(([_, data]: [string, any]) => data.gastos > 0)
      .sort(([_, a]: [string, any], [__, b]: [string, any]) => b.gastos - a.gastos);
    ...
    return {
      ...
      datasets: [
        {
          data: categorias.map(([_, data]: [string, any]) => data.gastos),
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {Object.entries(stats.statsPorCategoria)
                  .sort(([_, a]: [string, any], [__, b]: [string, any]) => Math.abs(b.total) - Math.abs(a.total))
  ...
  <div style={{ 
    ...
    {pdfResult.transactions && pdfResult.transactions.length > 0 ? (
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 10 }}>
        ...
        <tbody>
          {pdfResult.transactions.map((txn: any, idx: number) => (
            <tr key={idx}>
